...
combined <- read_excel("r_codes/data.xlsx", sheet = 1)
...
superplot <- ggplot(combined, aes(x = variable, y = value, color = factor(replicate))) +
    ...
    stat_summary(
        data = TotalAverages, fun.y = mean, fun.ymin = mean, fun.ymax = mean,
        geom = "crossbar", width = 0.25, color = "black"
    ) +

    # Adds error bars
    stat_summary(
        data = ReplicateAverages, fun.data = mean_se,
...
library(SuperPlotR)
superplot2 <- superplot(lord_jcb, "Speed", "Treatment", "Replicate", ylab = "Speed (µm/min)", size = c(0.8, 1.5), fsize = 9)

superplot2 + theme_minimal() +
    geom_signif(
        comparisons = list(c("Control", "Drug")),
        map_signif_level = TRUE, textsize = 6
    )

ggsave("r_codes/fig.jpg", width = 88, height = 50, units = "mm")
